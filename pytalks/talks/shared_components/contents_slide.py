...
def _build_unrolled_rnn(num_steps):
    """Build an N-step unrolled RNN spread across the full scene width."""
    # Scale spacing down for more steps to fit the screen
    spacing = min(3.2, 12.0 / max(num_steps, 1))
...
def make_contents_slide(scene: Scene, bullets: list[str]):
    """Render a contents slide with an unrolled RNN background and bullet points.

    The number of RNN steps matches the number of bullets.
    Each bullet appears alongside the corresponding RNN cell activating.
    """
    num_steps = len(bullets)
    ...
    bullet_group = VGroup()
    for text in bullets:
        bullet = Text(f"\u2022  {text}", font_size=font_size, color=WHITE)
        bullet_group.add(bullet)
    bullet_group.arrange(DOWN, aligned_edge=LEFT, buff=buff)
    bullet_group.next_to(title, DOWN, buff=0.5)
