...
from talks.shared_components.rnn_diagrams import build_unrolled_rnn_seq_labelling

_DIM = 0.15


def slide_rnn_seq_labelling(scene: Scene):
    """Standalone slide showing an unrolled RNN for sequence labelling with progressive activation."""
    scene.frame.move_to(ORIGIN)
    scene.frame.set_height(8)

    title = Text("RNN for Sequence Labelling", font_size=48, weight=BOLD)
    title.to_edge(UP, buff=0.5)

    words = ["The", "grand", "car"]
    labels = ["Article", "Adjective", "Noun"]
    unrolled, h0_group, steps = build_unrolled_rnn_seq_labelling(
        ORIGIN + DOWN * 0.3, words, labels
    )
