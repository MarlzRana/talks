...
def _build_background_network():
    """Build a wide, faded multi-layer network. Returns (network, neurons_by_layer, edges_by_layer, layer_colors)."""
...
def make_title_slide(scene: Scene, part: int):
    """Render the series title slide for the given part number."""
    ...
    # Save light state before color pulses so we can restore after
    ...
    for i, layer_nodes in enumerate(neurons_by_layer):
        ...
        fire_light_change(color, transition_ds=4)
    ...
    # Profile picture
    shared_images = Path(__file__).parent.parent / "shared-images"
    circular_path = _make_circular_image(str(shared_images / "profile_picture.png"))
    ...
    part_text.next_to(title, DOWN, buff=0.4)
    author.next_to(part_text, DOWN, buff=0.8)

    scene.play(FadeIn(profile_pic, shift=RIGHT * 0.3), run_time=0.8)
    scene.play(Write(title), run_time=1.5)
    scene.play(FadeIn(part_text, shift=UP * 0.3), run_time=0.8)
    scene.play(FadeIn(author, shift=UP * 0.2), run_time=0.8)

    # Restore light to its original state after the color pulses
    fire_light_restore(saved_light, transition_ds=10)
