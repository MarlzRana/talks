...
def _highlight(scene, bright_keys, groups, subtitle_text, prev_subtitle, circle_symbols, light_color=None):
...
def slide_transformer_overview(scene: Scene):
    ...
    # Save light state before highlights
    ...
    # Restore light before activation demo
    ...
    # Color mapping for sweep steps
    _enc_colors = [PINK, WHITE, ORANGE, YELLOW, BLUE, YELLOW]
    _dec_colors = [PINK, WHITE, ORANGE, YELLOW, ORANGE, YELLOW, BLUE, YELLOW, PURPLE, GREEN]

    def _sweep_stack(steps, syms_for_pos, step_colors, run_time=0.15):
        """Sweep a highlight through steps with light color changes."""
        for i, step in enumerate(steps):
            anims = [step.animate.set_opacity(1.0)]
            if i == 1:  # positional step
                for sym in syms_for_pos:
                    anims.append(sym.animate.set_color(BLACK).set_opacity(1.0))
            fire_light_change(step_colors[i], transition_ds=2)
    ...
    _sweep_stack(encoder_steps, enc_circle_syms, _enc_colors, run_time=0.12)

    # Decoder activates five times (auto-regressive)
    for _ in range(5):
        _sweep_stack(decoder_steps, dec_circle_syms, _dec_colors, run_time=0.1)

    # Restore light at the end
    fire_light_restore(saved_light, transition_ds=8)
